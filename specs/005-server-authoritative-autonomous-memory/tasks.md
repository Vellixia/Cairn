...
- [ ] T005 Implement SQLite schema v8 exactly as `data-model.md` §5—including `event_spool`, durable event/command sequences, `command_spool`, disposition counts, `authority_mode`, `retained_local`, `migration_state`, and immutable `legacy_pattern_claims`—in `crates/cairn-store/migrations/0008_safe_events.sql`
- [ ] T006 Test v7→v8 migration, interrupted migration rollback, existing-row preservation, retained-local discriminator constraints, and same-owner/different-owner legacy-pattern claim uniqueness in `tests/tests/feature005_local_schema.rs` (depends on T005)
- [ ] T007 Implement PostgreSQL schema v4 exactly as `data-model.md` §6—including authority mode, safe events, consolidation leases/work/runs/candidates, provenance, cross-domain FTS, traces, delivery dedup, reports/summaries, personal-domain patterns, health, and dispositions—in `crates/cairn-server/migrations/0004_autonomous_memory.sql`
- [ ] T008 Test v3→v4 migration, every foreign key/index/CHECK, canonical `reference_key`, nullable project bindings, owner-only pattern columns, and server-authority initialization in `tests/tests/feature005_server_schema.rs` (depends on T007; SC-766)
- [ ] T009 [P] Add server schema v4 and the settled capability advertisement without weakening older capability behavior in `crates/cairn-server/src/version.rs` (depends on T007)
- [ ] T010 Define `KnowledgeRef(domain,id)`, `PatternRef(pattern_id)`, the polymorphic reference enum, canonical `reference_key`, domain/type validation, and per-domain resolution contracts in `crates/cairn-core/src/domain.rs` (FR-708c–e, FR-817–FR-826, FR-819a)
- [ ] T011 [P] Define the closed, versioned 21-kind `SafeCanonicalEvent` union, typed per-kind content, provenance, dispositions, refusal vocabularies, and all numeric bounds in `crates/cairn-core/src/event.rs` (FR-734–FR-745)
- [ ] T012 [P] Implement daemon-assigned UUIDv5 event, command, candidate, refusal, corroboration, and pattern identities from their approved inputs in `crates/cairn-core/src/eventid.rs` (FR-708f, FR-738, FR-796–FR-798c)
- [ ] T013 Export the new domain, event, and identity APIs without changing the existing public tool surface in `crates/cairn-core/src/lib.rs` (depends on T010–T012)
- [ ] T014 Test the 21-kind closed union, old seven-event compatibility, per-kind content legality, bounds, clock-independent repeated-event identity, retry stability, and full reference encoding in `crates/cairn-core/tests/feature005_events.rs` (depends on T010–T013)
- [ ] T015 Extend the one canonical topic/value-key normalizer with approved separator folding while preserving dot-segment semantics and rejecting—not repairing—invalid keys in `crates/cairn-core/src/knowledge.rs` (FR-796a–d, FR-824)
- [ ] T016 Test at least fifty key-variant groups, invalid-key refusal, and unchanged topic dot segmentation in `crates/cairn-core/tests/feature005_keys.rs` (depends on T015; SC-745)
- [ ] T017 Extend the existing single privacy validator for safe-event text, `repo_file`, patterns, and consolidation candidates while keeping it pure, deterministic, fail-closed, and structurally text-free on refusal in `crates/cairn-core/src/validate.rs` (FR-749–FR-764, FR-777–FR-777g)
- [ ] T018 Test every privacy rejection class, missing-input refusal, POSIX/Windows path attacks, 1024-byte/64-segment limits, and refusal Debug/Display non-leakage in `crates/cairn-core/tests/feature005_privacy.rs` (depends on T017; SC-704, SC-705, SC-743)
- [ ] T019 Extend canonical lifecycle translation so all existing seven lifecycle events remain expressible through `SafeCanonicalEvent` in `crates/cairn-core/src/lifecycle.rs` (depends on T011)
- [ ] T020 Implement durable ordinal allocation, transactional identity assignment, exact-account claims, stale-claim reclaim, bounded backoff, permanent refusal, and capacity policy shared by event and command spools in `crates/cairn-store/src/spool.rs` (depends on T005, T012)
- [ ] T021 Export spool repositories and replace insert-once personal/team replica merging with server-wins cache refresh semantics in `crates/cairn-store/src/lib.rs` (depends on T020; FR-712a)
- [ ] T022 Test concurrent ordinal allocation, drain-independent identity, exact-account claims, stale reclaim, retry bounds, boundary-row protection, saturation/drop counters, and command ordering in `crates/cairn-store/tests/feature005_spool.rs` (depends on T020–T021)
- [ ] T023 Add authenticated per-domain reference resolution and ownership/membership guards—including owner-only `PatternRef` resolution and withheld-not-opaque behavior—to `crates/cairn-server/src/auth.rs` (depends on T007, T010; FR-768–FR-769a, FR-834, FR-846a)
- [ ] T024 Write failing command-boundary contract tests for intent-only payloads, credential-derived attribution, cross-member overwrite refusal, idempotent command retries, personal/pattern ownership, and atomic team transitions in `tests/tests/feature005_commands.rs` (depends on T002, T007, T023)
- [ ] T025 Implement the post-cutover knowledge command boundary—including project create/supersede/reinforce/relate/pin/forget, personal create/forget, team propose, pattern promote/forget, and no client-writable derived state—in `crates/cairn-server/src/commands.rs` (depends on T024; FR-701, FR-712, FR-815–FR-816)
- [ ] T026 Wire command endpoints and reuse the existing compare-and-swap ratify/retire handlers in `crates/cairn-server/src/api.rs` (depends on T025; FR-825, FR-889a)
- [ ] T027 Route explicit daemon/CLI knowledge mutations through online commands or the account-bound command spool, preserving sessionless store-scoped identity, in `crates/cairnd/src/handlers.rs` (depends on T020, T025)
...
- [ ] T029 Implement `POST /api/events/batch` validation and atomic `safe_events` plus `consolidation_work` persistence in `crates/cairn-server/src/events.rs` (depends on T028; FR-765–FR-780)
- [ ] T030 Wire strict body limits, authentication, routing, and per-event outcomes for event ingest in `crates/cairn-server/src/main.rs` (depends on T029)
...
- [ ] T035 Define the complete per-agent/per-event capability matrix vocabulary, evidence kind, no-evidence state, and per-machine attribution in `crates/cairn-integrate/src/capability.rs` (FR-728–FR-729, FR-838a–f, FR-851–FR-860)
...
- [ ] T039 Add the foundation modules to their crate registries and preserve the two-process architecture in `crates/cairn-server/src/main.rs` (depends on T009–T038)
...
- [ ] T055 [US1] Extend project/personal/team PostgreSQL full-text search and exact normalized-key lookup without embeddings in `crates/cairn-server/migrations/0004_autonomous_memory.sql` (depends on T007, T015; FR-806)
...
- [ ] T058 [US1] Implement client event-spool draining in session-sequence order with per-item accepted/duplicate/refused handling and supported-version deferral in `crates/cairnd/src/sync.rs` (depends on T020, T029; FR-770–FR-775)
...
- [ ] T066 [P] [US2] Write failing trace/privacy tests for complete considered/selected refs, failures, 90-day retention, no briefing text, owner withholding, dense post-filter ranks, scoped budgets, and membership refusal in `tests/tests/feature005_retrieval_traces.rs` (depends on T023, T034; SC-729, SC-761)
- [ ] T067 [P] [US2] Write failing delivery tests for Claude/Codex session-open, prompt-time, compact-triggered reopen, no post-compact return, OpenCode decline, no confirmed acknowledgements, and no tool call in `tests/tests/feature005_delivery.rs` (depends on T035; SC-708, SC-712)
...
- [ ] T070 [US2] Persist retrieval traces/items and delivered-context rows only after non-failed transmission attempts, using `reference_key` identities and bounded retention sweeps in `crates/cairn-server/src/retrieve.rs` (depends on T066, T069; FR-839–FR-850)
- [ ] T071 [US2] Add authenticated `POST /api/retrieve` with session-derived project/account identity and explicit-call override semantics in `crates/cairn-server/src/api.rs` (depends on T069–T070)
- [ ] T072 [US2] Implement daemon delivery orchestration, deadline degradation, transmission telemetry, and account-bound bounded outage cache in `crates/cairnd/src/deliver.rs`, then register it in `crates/cairnd/src/main.rs` (depends on T068, T071)
...
- [ ] T078 [US2] Make T066 trace/privacy tests pass without opaque references, rank gaps, cross-account patterns, or persisted briefing text in `tests/tests/feature005_retrieval_traces.rs` (depends on T070–T071)
- [ ] T079 [US2] Make T067 delivery-point tests pass for every committed point and keep acknowledgement `unavailable / no evidence` in `tests/tests/feature005_delivery.rs` (depends on T072–T075)
...
- [ ] T083 [P] [US3] Write failing local-loss tests for project/personal/team/pattern restoration from directly seeded server records, cache-empty/stale reporting, and exact lost categories in `tests/tests/feature005_local_loss.rs` (depends on T025, T034; SC-713, SC-714, SC-738)
- [ ] T084 [P] [US3] Write failing owner-only pattern lifecycle tests for safe promotion shape, deterministic duplicate identity, trust narrowing, forget tombstone, retrieval/traces/API isolation, and team widening only through a separate proposal in `tests/tests/feature005_patterns.rs` (depends on T025, T034; SC-760–SC-762)
- [ ] T085 [US3] Implement safe personal-domain pattern create/list/forget repositories with credential-bound owner, deterministic content identity, `sanitized` server trust, and no local-only fields in `crates/cairn-server/src/commands.rs` (depends on T084)
...
- [ ] T096 [US4] Add bounded asynchronous event and command drain loops with exponential backoff and stale-claim recovery to `crates/cairnd/src/sync.rs` (depends on T058, T093)
...
- [ ] T108 [US5] Implement bounded membership-guarded funnel, activity, consolidation-run, retrieval-trace, integration-health, and system-health read handlers in `crates/cairn-server/src/api.rs` (depends on T106; FR-879–FR-882, FR-886–FR-887, FR-891, FR-894–FR-895)
...
- [ ] T130 [US6] Queue privacy-safe verification reports as commands while keeping raw evidence facts/runs local in `crates/cairnd/src/verify.rs` (depends on T097, T129; FR-707, FR-811c)
...
- [ ] T132 [US6] Persist authenticated health/disposition reports and expose stale/no-evidence semantics in `crates/cairn-server/src/api.rs` (depends on T108, T131)
...
- [ ] T141 [P] [US7] Write failing cutover compatibility tests for pre-cutover migration, post-cutover permanent `upgrade_required`, untouched legacy local data, ordinary local operation, upgraded-client retry stop, and server-instance binding in `tests/tests/feature005_cutover.rs` (depends on T009, T138; SC-746, SC-747)
- [ ] T142 [P] [US7] Write failing cutover verification tests for server demotion/audit counts, cleared unsupported values, untouched client states, and changes-feed omission in `tests/tests/feature005_verification_cutover.rs` (depends on T126; SC-763)
- [ ] T143 [US7] Implement resumable inspect, explicit pattern claim, per-author drain, possession, atomic authority switch, possession recheck, demotion, and retained retry phases in `crates/cairnd/src/migrate005.rs`, then register them in `crates/cairnd/src/main.rs` (depends on T138–T142; FR-861–FR-878)
...
- [ ] T146 [US7] Enforce exact per-author eligibility for legacy rows and persisted claimant eligibility for patterns during drain in `crates/cairnd/src/sync.rs` (depends on T139, T143; FR-864a, FR-867b)
- [ ] T147 [US7] Normalize both legacy key kinds through the shared normalizer and record—not discard—collisions through existing conflict machinery in `crates/cairnd/src/migrate005.rs` (depends on T140, T143; FR-867a)
- [ ] T148 [US7] Implement admin compare-and-swap cutover, authority advertisement, and server-wide unsubstantiated verification demotion/audit in one transaction in `crates/cairn-server/src/api.rs` (depends on T142, T145; FR-811e–g, FR-876)
- [ ] T149 [US7] Refuse post-cutover knowledge-bearing sync shapes with permanent `upgrade_required` while preserving non-knowledge sync and all read feeds in `crates/cairn-server/src/sync.rs` (depends on T141, T148; FR-876a–e, FR-877)
- [ ] T150 [US7] Stop emitting server verification objects on the post-cutover changes feed so local run-derived state is never demoted remotely in `crates/cairn-server/src/sync.rs` (depends on T142, T148)
...
- US4 (T093–T105) depends only on Foundations; its independent outage path exercises fresh-unavailable behavior, while cached briefing behavior remains independently covered in US2.
...
- US6 (T125–T137) depends on Foundations and uses server-seeded records; it does not depend on web UI.
