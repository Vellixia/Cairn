...
tests/knowledge/
├── reconciliation/
│   ├── equivalent/          ≥20 cases — same subject, equal value → exactly one answer
...
| 2 | False merges on `distinct/` + `free_form/` | **0** | `us1_reconciliation` | SC-301 |
...
| 12 | Level 0 retention at the minimum budget with 5,000 memories | 100% | `us10_min_safe_context` | SC-309 |
...
| 25 | Criterion never `verified` on attested evidence alone | 100% | `us11_task_criteria` | SC-328 |
...
| 34 | Degraded sync against a schema-1 server delivers everything accepted | 100% | `sync_degradation` | SC-326 |
