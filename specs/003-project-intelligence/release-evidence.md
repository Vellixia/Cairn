...
| Deterministic gates | passing — **1,088 passed, 0 failed, 1 ignored, 0 skipped**, against real PostgreSQL 18.4 with `--all-targets` |
...
| Open items, recorded and deliberately not fixed | **one, filed, unchanged in scope.** [#44](https://github.com/Vellixia/Cairn/issues/44) — on a pull, the client drops a record it cannot yet place rather than retrying it, while the server's comment says it holds and retries. With push ordering corrected and `page_cursor` pinning behind a truncated page, the reachable exposure is gone; closing it properly needs `updated_at` on the wire and a policy for a record that can never be placed — a redesign the issue itself scopes as out of bounds absent a demonstrated reachable failure, and none has been demonstrated. Checkpoint T added two debug-level log lines at the decline points so the gap is at least diagnosable; the mechanism, the cursor and the wire shape are unchanged. Written up in Checkpoint R and Checkpoint T. |
