# Cairn â€” The Open-Source Context & Reliability Layer for AI Agents

> **Make any model smart.** Remember everything Â· feed less, not more Â· stay reliable on long
> tasks Â· get smarter together â€” self-hosted, one Rust binary, with no context ever lost.
...
### The four references (ideas only â€” no forking)

| Project | Lang | What we take |
|---|---|---|
| **agentmemory** (rohitg00) | TS/iii | 4-tier memory (workingâ†’episodicâ†’semanticâ†’procedural), consolidation/decay, hybrid recall (BM25+vector+graph, RRF), lifecycle hooks, viewer |
| **lean-ctx** (yvgude) | Rust | 10 read modes, ~13-tok cached re-reads, tree-sitter AST (11 langs), property graph, `serve` HTTP-MCP |
| **rtk / Rust Token Killer** (rtk-ai) | Rust | Command-output compression (filter/group/dedup) via hooks, 100+ filters, **tee/recover**, single binary, ~80% session cut â€” proves the Rust+hook approach |
...
**Name:** **Cairn** â€” a stack of trail-marker stones. Travelers each add a stone, everyone who
follows benefits (**collective knowledge**); each session leaves a marker the next one follows
(**memory**); a cairn is minimal â€” only the stones needed to navigate (**lean, no-loss context**).
...
- **Audience:** **open-source, self-hosted** â€” public repo, many self-hosters, **federation**
  between servers, **high polish + a high privacy/sanitization bar.**
- **Posture:** **active guardrails** â€” Cairn doesn't just feed context, it *verifies* agent
  output against retained originals, detects drift, and re-anchors long tasks. (Not full
  multi-agent orchestration in v1 â€” see Scope.)
- **Hero:** *all of it* â€” five pillars, unified. Umbrella message: "the context & reliability
...
**The throughline: the bottleneck is the *context fed to the model* and the *drift over time* â€”
...
**A. Context-window failures (architectural â€” bigger windows don't help):**
- **Context rot** â€” Chroma tested 18 frontier models; *all* degrade as input grows, even on
  trivial tasks; quality "drops off a cliff" past ~50% fill. Cause is attention math (RoPE decay
  + softmax), not retrieval. â†’ *feed less + ordered, don't just compress.*
- **Lost-in-the-middle** â€” mid-context info is ignored; models favor the edges. â†’ *put critical
  facts at the start/end.*
- **Lossy compaction** â€” auto-summarization silently drops decisions, constraints, gotchas.

**B. Long-horizon failures (temporal â€” the active-guardrails target):**
- **Error compounding / reasoning drift** â€” small per-step errors snowball to systematic
  failure; silent, "no stack trace, no alert" (Microsoft: models "can't handle long-running tasks").
- **Silent corruption when delegating** â€” frontier models lost ~25% of document content over 20
  delegated edits. â†’ *verify output against a retained ground truth.*
- **Reliability â‰  pass@1** â€” production needs reliability across many steps.

**C. Continuity failures (state):** cross-session amnesia Â· cross-device silos Â· cross-agent
silos Â· re-reading unchanged files Â· re-explaining conventions Â· lost rationale Â· no task resume.

**D. Knowledge/capability failures (collective):** weak/cheap models underperform for lack of
context (not IQ) Â· knowledge siloed per user Â· everyone re-learns the same lessons Â· the agent-
memory market is hot but **memory-only** (MemPalace, mem0â€¦) â€” no unified, self-host, no-loss,
reliable, collective solution.

**E. Multi-agent (scoped out of v1):** 41â€“86% failure rates, mostly *coordination*, not
capability; "context collapse" is the top long-task killer; single-agent often *beats*
multi-agent. â†’ Cairn targets **single-agent reliability first**; shared-context primitives later.

---

## Core Principles

1. **No context loss â€” lossless by retention.** Cairn is a stateful server: every compression
   ...
   demand** (`expand`/`recover`). Window shrinks 60â€“90%; the system loses nothing. (Beats rtk's
   stateless re-exec and caveman's irreversible loss.)
2. **Less, not more â€” anti-rot.** Don't dump context. **Assemble** the minimal, highest-signal,
...
## Product Vision â€” five pillars

```mermaid
graph TD
    Root["Cairn â€” 5 pillars"]
...
1. **Remember** â€” never start cold; decisions/tasks/rationale persist across sessions, devices, agents.
2. **Compress without loss** â€” files, shell output, responses shrink in the window, stay fully recoverable.
3. **Assemble lean context** â€” fight context rot: feed less, higher-signal, well-ordered context.
4. **Stay reliable** â€” verify edits vs originals, detect drift, re-anchor long tasks (active guardrails).
5. **Get smarter together** â€” learn each user's preferences + opt-in **collective knowledge** so
...
- **Palette:** Ink `#0B0F14` Â· Surface `#12181F` Â· Slate `#8A94A6` Â· Off-white `#ECEFF4` Â· Accent
  ember `#FB923C` Â· Signal teal `#2DD4BF`.
- **Type:** Geist Sans + Geist Mono (Inter + JetBrains Mono fallback).
- **Voice:** precise, calm, wayfinding â€” "leave a marker," "only the stones you need," "never
...
| Shell/tool output | rtk filter/group/dedup | `cairn-shell` | `recover` (teeâ†’blob store) |
...
## Web Control Plane (operational UI) â€” you *do* things here, not just watch

One web app (also serves the landing site) = the single pane of glass to **install, connect,
configure, inspect, edit, approve, share, and debug**. Real-time (WS), keyboard-first (**âŒ˜K
...
- **Setup wizard (first run):** create account â†’ pick embedding provider â†’ **Add Device**
  (copy-paste installer + QR/pairing code) â†’ **Connect Agents** (one-click per detected agent) â†’
  green health check.
...
- **Memory workspace (editable):** search; **create / edit / pin / delete** memories; mark
  important; resolve contradictions; view+edit rationale; promote workingâ†’semantic; bulk actions.
- **Profile editor:** view / approve / edit learned preferences + do/don't rules.
- **Assembler playground (inspector):** type a query + token budget â†’ see exactly what context
...
- **Savings & recover:** tokens/$ saved, signed savings ledger, and **expand/recover any
  compressed artifact** â€” for trust + debugging.
...
## Install & Onboarding (dead-simple â€” the user's priority)
...
- **Repo:** monorepo â€” Cargo workspace + `/web` (Next.js) + `/docs`.
...
- **Scope discipline:** multi-month product. Parallel tracks, but each ships a **thin** slice
  before depth. **Multi-agent orchestration is out of v1** (single-agent reliability first â€” the
  research shows single-agent often wins and coordination is where multi-agent fails).
- **Privacy of collective/federation is the #1 risk** â€” sanitization + consent + provenance +
...
- **Rust maturity:** confirm `rmcp` + `fastembed`; `automerge` sync + federation are the most
  novel pieces â€” prototype early (Phase 2/3).
...
- [Architecture](ARCHITECTURE.md) â€” how the code is structured today (crate graph, data flow, tool surface)
- [Roadmap](ROADMAP.md) â€” what's done, what's in progress, what's next
- [Benchmarks](BENCHMARKS.md) â€” methodology + measured numbers + targets
- [Audit Report](audits/REPORT.md) â€” security audit with fix-status tracking