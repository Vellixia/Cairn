...
- **Findings** go to `web/test/findings/<slug>.md`. The format is the existing one (Severity / Discovered / What happened / Steps / Expected / Actual / Evidence / Suggested fix). Reuse, don't reinvent.
...
| **Result** | PASS or FAIL. FAIL must reference a finding file in `web/test/findings/`. |
...
7. If any FAIL, write web/test/findings/<NN>-<surface>-<short-desc>.md per the
...
## Findings
<link to web/test/findings/<slug>.md if any>
```
