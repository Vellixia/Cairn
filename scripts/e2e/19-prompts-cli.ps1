...
# `cairn stats` may not exist in this version; accept any
# non-empty output as a sign the binary works.
Assert-True -Condition ($out2.Length -gt 0) -Msg 'cairn stats produces output'

$out3 = Invoke-CairnCli export
Assert-True -Condition ($out3.Length -gt 0) -Msg 'cairn export produces output'
