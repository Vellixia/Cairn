...
//! knowledge`, `/api/team/knowledge` and `/api/patterns`,
...
// ---------------------------------------------------------------------------
// The three screened entry points, each reduced to "was this text refused, and
// as what"
// ---------------------------------------------------------------------------
...
/// The class at the tail of a command-route refusal message.
...
/// parser that filtered them by value would be a hole: content saying `com`
/// would then name that project undetected.
///
...
// ---------------------------------------------------------------------------
// 5. Which project rows command routes screen against (FR-577a)
// ---------------------------------------------------------------------------
...
fn memberships(server: Server) -> Fixture {
    ...
    // A soft delete leaves `name` and `repository_remote` in place. The tokens
    // are therefore still there to screen with.
    server.execute(&format!(
...
///
/// **Falsified by** the screen filtering `deleted_at IS NULL`, which would
/// make this project's name ordinary prose.
#[test]
