...
- **Hard assertions:** every step that lists `Assert:` must be verified against the accessibility snapshot. If the assertion fails, the flow writes a markdown finding under `web/test/findings/<slug>.md`.
...
Save to `web/test/findings/<flow-slug>-<short-desc>.md`.